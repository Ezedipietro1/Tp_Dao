	...
	export function Reportes() {
	  ...
	  useEffect(()=>{ clientesService.Buscar().then(d=>setClientes(d||[])).catch(e=>console.error(e)); }, []);
	  ...
	  return (
	    <div className="reportes-page container py-3">
	      ...
	      <div className="mb-3 d-flex gap-2">
	        ...
	        <button className={`btn btn-info`} onClick={()=>{ const anio = prompt('Año (YYYY) para el reporte de utilización mensual:'); if (anio) { setMode('utilizacion-mensual'); fetch(`${API_BASE}/reportes/json/utilizacion/${encodeURIComponent(anio)}`).then(r=>r.json()).then(data=>{ const counts = data.counts||[]; setCanchaData(counts.map((c,i)=>({month:i+1,count:c}))); const labels = counts.map((_,i)=>['Ene','Feb','Mar','Abr','May','Jun','Jul','Ago','Sep','Oct','Nov','Dic'][i]); updateChart(labels, counts, `Utilización ${anio}`); }).catch(e=>{console.error(e); alert('Error cargando utilización mensual'); }); } }}>Utilización mensual de canchas</button>
